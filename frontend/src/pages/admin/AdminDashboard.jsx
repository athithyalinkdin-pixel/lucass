...
const AdminDashboard = () => {
  ...
  const handleMarkRead = async (messageId) => {
    try {
      ...
      setStats((prevStats) => ({
        ...prevStats,
        unreadMessages: Math.max(0, (prevStats?.unreadMessages || 1) - 1),
      }));
    } catch (err) {
    ...
    <div className="pt-32 pb-24 min-h-screen bg-bg-off-white">
      ...
      <div className="p-8 max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-10">
          <div>
            <h1 className="text-3xl font-bold text-primary mb-2 font-serif">Dashboard Overview</h1>
          ...
          <button
            ...
            className="flex items-center gap-2 text-red-500 font-bold hover:bg-red-50 px-4 py-2 rounded-xl transition-colors border border-red-100"
          >
            <LogOut size={18} />
            <span>Logout</span>
          </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {statCards.map((card, idx) => (
            <motion.div
              ...
              key={idx}
              className={`p-6 rounded-3xl ${card.color} border border-primary/5 flex items-center gap-6 shadow-sm bg-white`}
            >
              <div className="p-4 bg-white/80 rounded-2xl shadow-sm">{card.icon}</div>
              <div>
        ...
        <div className="grid lg:grid-cols-3 gap-10">
          {/* Recent Orders List */}
          <div className="lg:col-span-2 glass-card p-8 border border-accent/20">
            <div className="flex justify-between items-center mb-8">
              <h3 className="text-xl font-bold text-primary font-serif">Recent Orders</h3>
              <Link to="/admin" className="text-sm font-bold text-secondary hover:underline">
                View All
              </Link>
            </div>
                    ...
                    <tr key={order.id} className="group hover:bg-bg-off-white transition-colors">
                      ...
                      <td className="py-4 text-sm">
                        <select
                          ...
                          className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase border border-accent/30 focus:ring-2 focus:ring-secondary cursor-pointer ${
          ...
          <div className="glass-card p-8 border border-accent/20">
            ...
            <div className="grid gap-3">
              ...
              <Link
                ...
                className="flex items-center gap-4 p-4 rounded-2xl bg-primary/5 hover:bg-primary hover:text-white transition-all group border border-accent/20"
              >
                <Award className="text-secondary group-hover:text-white" size={20} />
                <span className="font-bold text-sm">Manage Homepage Products</span>
              </Link>
              <Link
                to="/admin/blog"
                className="flex items-center gap-4 p-4 rounded-2xl bg-primary/5 hover:bg-primary hover:text-white transition-all group border border-accent/20"
              ...
              >
                ...
                <span className="font-bold text-sm">Manage User Roles</span>
              </Link>
              <div className="flex items-center gap-4 p-4 rounded-2xl bg-amber-50 border border-amber-100">
                ...
                <span className="font-bold text-sm text-amber-700">
                  {stats?.lowStock || 0} Low Stock Alerts
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="mt-10 glass-card p-8 border border-accent/20">
          <div className="flex justify-between items-center mb-8">
          ...
          <div className="space-y-4">
            {messages.length === 0 ? (
              <p className="text-center text-primary/40 py-8 text-sm font-bold">No messages received yet.</p>
            ) : (
              messages.map((msg) => (
                <div
                  key={msg.id}
                  className={`p-5 rounded-2xl border transition-colors ${
                    msg.is_read ? 'border-accent bg-transparent' : 'border-secondary/30 bg-secondary/5'
                  }`}
                >
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      {msg.subject && (
                        <p className="text-sm font-bold text-primary/70 mt-2">{msg.subject}</p>
                      )}
